...
class ApiExplorer extends Loggable {
  ...
  def showResources = {
    ...
    val (catalogHeadline, catalogDescription) = List(showCore, showOBWG, showPSD2)  match {
      ...
      // Core
      case List(Some(true), None, None) => ("Core Finocial",
      ...
      // Non Core
      case List(Some(false), None, None) => ("Non-Core Finocial",
        "These Finocial APIs don't assume the owner of the account is accessing the account related resources (so they support accountant, auditor, public, admin access) " +
          "Meta data, data sharing, datar redaction and entitlements is included. ")

      // All
      case List(None, None, None) => ("Finocial",
        "The full set of Finocial APIs supports functionality including transaction history, payments, onboarding & KYC, cards, customer and customer messages, counterparty and transaction metadata, delegated account access, data redaction and entitlements.")

      // UK OBWG
      case List(None, Some(true), None) => ("Indian Banking",
      ...
      // Intersection
      case List(Some(true), Some(true), Some(true)) => ("Intersection of Finocial Core, Indian Banking and PSD2",
